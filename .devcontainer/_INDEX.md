...
> Base devcontainer for the kernel — generic Node 20 + mise + zsh + Claude Code scaffold plus an optional hermes agent-harness image, designed for extension by downstream projects
...
| Name | Description |
|------|-------------|
| `Dockerfile.base` | Multi-stage base image — `kernel-base` (Node 20, mise, git-delta, zsh, Claude Code), `hermes-deps` (+ hermes-agent venv from the pinned public repo), `hermes` (+ entrypoint/config layer) |
...
| `init-firewall.sh` | iptables/ipset firewall init script — resolves allowed domains from firewall.d/_required/ and firewall.d/ drop-ins |
